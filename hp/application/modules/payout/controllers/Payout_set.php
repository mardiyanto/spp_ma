...
class Payout_set extends CI_Controller
{
  ...
  public function __construct()
  {
    ...
    $this->load->model(array('payment/Payment_model', 'student/Student_model', 'period/Period_model', 'pos/Pos_model', 'bulan/Bulan_model', 'bulan/Bulan_pay_model', 'bebas/Bebas_model', 'bebas/Bebas_pay_model', 'setting/Setting_model', 'letter/Letter_model', 'logs/Logs_model', 'ltrx/Log_trx_model'));
  }
...
// Cetak Nota pembayaran harian
  ...
  public function payout_bebas($id = NULL, $student_id = NULL, $bebas_id = NULL, $pay_id = NULL)
  {
    if ($_POST == TRUE) {
      ...
      $param['bebas_pay_input_date'] = $tgl;
  ...
  function not_pay($payment_id = NULL, $student_id = NULL, $id = NULL)
  {
    ...
    $this->db->where('bulan_bulan_id', $id);
...
// Delete to database
public function hapus_bulan_pay($payment_id = NULL, $student_id = NULL, $bulan_id = NULL, $id = NULL)
{     
  $student = $this->Bulan_pay_model->get(array( 'student_id' => $student_id, 'bulan_id' => $bulan_id, 'id' => $id));
  ...
  $p = $this->db->query("SELECT SUM(bulan_pay_bill) AS Total FROM bulan_pay where bulan_bulan_id = $bulan_id")->row_array();
  $tt = $p['Total'];
  $pay = array(
      'bulan_id' => $bulan_id,
      'bulan_bill_total' => $tt
  ...
  $this->db->where('bulan_id', $bulan_id);
  $this->db->update('bulan', $pay);  
  ...
  function update_pay_desc()
  {
    ...
    $bulan_bill_total = $this->input->post('bulan_bill_total');
    $bulan_pay_input_date = $this->input->post('bulan_pay_input_date');
    $bulan_pay_bill = $this->input->post('bulan_pay_bill');
    if ($lastletter['letter_year'] < date('Y') or count($lastletter) == 0) {
      $this->Letter_model->add(array('letter_number' => '00001', 'letter_month' => date('m'), 'letter_year' => date('Y')));
      $nomor = sprintf('%05d', '00001');
      $nofull = date('Y') . date('m') . $nomor;
    ...
    $data = array(
      'bulan_bulan_id' => $id,
      'bulan_pay_bill' => $bulan_pay_bill,
      'keterangan' => $bulan_pay_desc,
      'bulan_pay_number' => $nofull,
      'bulan_pay_input_date' => $bulan_pay_input_date,
      'bulan_pay_last_update' => date('Y-m-d H:i:s'),
      'user_user_id' => $this->session->userdata('uid'),
    );
    $this->db->insert('bulan_pay', $data);
    $p = $this->db->query("SELECT SUM(bulan_pay_bill) AS Total FROM bulan_pay where bulan_bulan_id = $id")->row_array();
    $tt = $p['Total'];
    $data = array(
      'bulan_id' => $id,
      'bulan_pay_desc' => $bulan_pay_desc,
      'bulan_bill_total' => $tt,
      'bulan_date_pay' => $bulan_pay_input_date,
    );
