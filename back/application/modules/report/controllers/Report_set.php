...
class Report_set extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model(array('payment/Payment_model', 'pos/Pos_joinmodel', 'guru/Guru_model', 'gaji/Gaji_model', 'potongan_pay/Potongan_pay_model', 'gaji_pay/Gaji_pay_model', 'student/Student_model', 'period/Period_model', 'pos/Pos_model', 'bulan/Bulan_model', 'bulan/Bulan_pay_model', 'bebas/Bebas_model', 'bebas/Bebas_pay_model', 'setting/Setting_model', 'kredit/Kredit_model', 'debit/Debit_model', 'logs/Logs_model'));
	}
	...
	public function index($offset = NULL)
	{
		...
		$data['gaji_paket'] = 0;
		$data['gaji_nonpaket'] = 0;
		foreach ($data['gaji_pay'] as $key) {
			if ($key['gaji_jenis']== 'paket' ) {
			$data['gaji_nonpaket'] += $key['gaji_pay_value'];
		} else {
			$data['gaji_paket'] += $key['gaji_pay_value']*$key['gaji_value'];
		}
		}
		$data['total'] = 0;
		foreach ($data['dom'] as $key) {
			$data['total'] += $key['bebas_pay_bill'];
		}
		$data['total_pay'] = 0;
		foreach ($data['bulan_pay'] as $row) {
			$data['total_pay'] += $row['bulan_pay_bill'];
		}
		$data['kredit_pay'] = 0;
		foreach ($data['kredit'] as $row) {
			$data['kredit_pay'] += $row['kredit_value'];
		}
		$data['debit_pay'] = 0;
		foreach ($data['debit'] as $row) {
			$data['debit_pay'] += $row['debit_value'];
		}
		// hitung pengeluaran
		$data['loss']  = $data['kredit_pay'] + $data['gaji_nonpaket'] + $data['gaji_paket'];
		$data['total_gaji']  = $data['gaji_nonpaket'] + $data['gaji_paket'];
		// hitung pendapatan
		$data['byr']  = $data['total'] + $data['total_pay'] + $data['debit_pay'];
		$data['bersih']  = $data['byr'] - $data['loss'];

		$data['title'] = 'LAPORAN KEUANGAN';
		$data['main'] = 'report/report_list';
		$this->load->view('manage/layout', $data);
	}
	public function neraca_get($offset = NULL)
	{ 
	// Apply Filter
		// Get $_GET variable
		$q = $this->input->get(NULL, TRUE);
		...
		$data['byr']  = $data['total'] + $data['total_pay'] + $data['debit_pay'];
		$data['bersih']  = $data['byr'] - $data['loss'];
		$data['title'] = 'NERACA KEUANGAN FULL';
		$data['main'] = 'report/report_getneraca';
		$this->load->view('manage/layout', $data);

	}
	public function neraca_get_pdf($offset = NULL)
	{ 
	// Apply Filter
		// Get $_GET variable
		...
		$data['byr']  = $data['total'] + $data['total_pay'] + $data['debit_pay'];
		$data['bersih']  = $data['byr'] - $data['loss'];
		$data['title'] = 'LAPOARAN NERACA';
		$html = $this->load->view('report/report_neraca_pdf', $data, true);
		$data = pdf_create($html, 'LAPORAN NERACA_' . date('d_m_Y'), TRUE, 'A4', 'potrait');

	}
	public function neraca($offset = NULL)
	{
		// Apply Filter
		// Get $_GET variable
		$q = $this->input->get(NULL, TRUE);
		...
		$data['main'] = 'report/report_neraca';
		$this->load->view('manage/layout', $data);
	}
	public function pdf_report_bill_bayar($offset = NULL)
	{
		// Apply Filter
		// Get $_GET variable
		$this->load->helper(array('dompdf'));
		...
		$data['byr']  = $data['total'] + $data['total_pay'];
		...
		$data['title'] = 'LAPOARAN PEMABAYARAN';
		$html = $this->load->view('report/report_list_bayar_pdf', $data, true);
		$data = pdf_create($html, 'LAPORAN PEMABAYARAN_' . date('d_m_Y'), TRUE, 'A4', 'potrait');
	}
	public function report_bill_bayar($offset = NULL)
	{
		// Apply Filter
		// Get $_GET variable
		$q = $this->input->get(NULL, TRUE);
		...
		$config['total_rows'] = count($this->Bulan_model->get($paramsPage));
		$data['title'] = 'LAPORAN KEUANGAN';
		$data['main'] = 'report/report_list_bayar';
		$this->load->view('manage/layout', $data);
	}
	
	public function gaji_report($offset = NULL)
	{
		// Apply Filter
		// Get $_GET variable
		$q = $this->input->get(NULL, TRUE);
		...
		$data['total'] = 0;
		foreach ($data['gaji_pay'] as $key) {
			$data['total'] += $key['gaji_pay_value'];
		}
		$config['base_url'] = site_url('manage/report/index');
		$config['suffix'] = '?' . http_build_query($_GET, '', "&");
		$config['total_rows'] = count($this->Bulan_model->get($paramsPage));
		$data['title'] = 'LAPORAN GAJI';
		$data['main'] = 'report/laporan_gaji';
		$this->load->view('manage/layout', $data);
	}
	public function gaji_report_guru($offset = NULL)
	{
		// Apply Filter
		// Get $_GET variable
		$q = $this->input->get(NULL, TRUE);
		...
		// Date start
		if (isset($q['gr']) && !empty($q['gr']) && $q['gr'] != '') {
			$params['guru_guru_id'] = $q['gr'];
		}
		// Date start
		if (isset($q['ds']) && !empty($q['ds']) && $q['ds'] != '') {
			$params['date_start'] = $q['ds'];
		...
		$config['base_url'] = site_url('manage/report/index');
		$config['suffix'] = '?' . http_build_query($_GET, '', "&");
		$data['title'] = 'LAPORAN GAJI GURU';
		$data['main'] = 'report/laporan_gaji_guru';
		$this->load->view('manage/layout', $data);
	}
	public function gaji_report_pdf()
	{
		// Apply Filter
		// Get $_GET variable
		$this->load->helper(array('dompdf'));
		...
		if (isset($q['gr']) && !empty($q['gr']) && $q['gr'] != '') {
			$params['guru_guru_id'] = $q['gr'];
			//$guru = $this->Guru_model->get(array('guru_guru_id' => $q['gr']));
		}
		// Date start
		if (isset($q['ds']) && !empty($q['ds']) && $q['ds'] != '') {
			$params['date_start'] = $q['ds'];
		...
		$paramsPage = $params;
		//$data['guru'] = $this->Guru_model->get(array('guru_id' => $guru['guru_guru_id'], 'group' => TRUE));
		//$data['guru'] = $this->Guru_model->get($params);
		$data['gaji'] = $this->Gaji_model->get($params);
		$data['gaji_pay'] = $this->Gaji_pay_model->get($params);
		$data['potongan_pay'] = $this->Potongan_pay_model->get($params);
		...
		$data['title'] = 'RINCIAN SLIP GAJI GURU';
		//$data['main'] = 'report/laporan_gaji_pdf';
		$html = $this->load->view('report/laporan_gaji_pdf', $data, true);
        $data = pdf_create($html, 'KARTU_' . date('d_m_Y'), TRUE, 'A4', 'potrait');
		//$this->load->view('manage/print', $data);
		//$html = $this->load->view('manage/print', $data);
		//$data = pdf_create($html, $guru['guru_nama'], TRUE, 'A4', TRUE);
		
	}
	public function pengeluaran($offset = NULL)
	{
		// Apply Filter
		// Get $_GET variable
		$q = $this->input->get(NULL, TRUE);
		...
		$data['kredit'] = $this->Kredit_model->get($params);
		$data['total'] = 0;
		foreach ($data['kredit'] as $key) {
			$data['total'] += $key['kredit_value'];
		}
		$config['base_url'] = site_url('manage/report/index');
		$config['suffix'] = '?' . http_build_query($_GET, '', "&");
		$config['total_rows'] = count($this->Bulan_model->get($paramsPage));
		$data['title'] = 'LAPORAN KEUANGAN';
		$data['main'] = 'report/pengeluaran';
		$this->load->view('manage/layout', $data);
	}
	public function pdf_pengeluaran($offset = NULL)
	{
		// Apply Filter
		// Get $_GET variable
		$this->load->helper(array('dompdf'));
		...
		foreach ($data['kredit'] as $key) {
		 ...
		 $data['title'] = 'LAPOARAN PENGELUARAN';
		 $html = $this->load->view('report/pengeluaran_pdf', $data, true);
		 $data = pdf_create($html, 'PENGELUARAN_' . date('d_m_Y'), TRUE, 'A4', 'potrait');
	}
	public function pdf_penerimaan($offset = NULL)
	{
		// Apply Filter
		// Get $_GET variable
		$this->load->helper(array('dompdf'));
		...
		$data['debit'] = $this->Debit_model->get($params);
		$data['total'] = 0;
		foreach ($data['debit'] as $key) {
			$data['total'] += $key['debit_value'];
		}
	 // endtotal
		$data['setting_district'] = $this->Setting_model->get(array('id' => SCHOOL_DISTRICT));
		$data['setting_city'] = $this->Setting_model->get(array('id' => SCHOOL_CITY));
		...
		$data['title'] = 'LAPOARAN PENERIMAAN';
		$html = $this->load->view('report/penerimaan_pdf', $data, true);
		$data = pdf_create($html, 'PENGELUARAN_' . date('d_m_Y'), TRUE, 'A4', 'potrait');
	}
	public function penerimaan($offset = NULL)
	{
		// Apply Filter
		// Get $_GET variable
		$q = $this->input->get(NULL, TRUE);
	...
	public function report()
	{
		...
		foreach ($data['bulan_pay'] as $row) {
			...
			$objSheet->setCellValue('E' . $cell, pretty_date($row['bulan_pay_input_date'], 'm/d/Y', FALSE));
			$objSheet->setCellValue('F' . $cell, $row['bulan_pay_bill']);
			$objSheet->setCellValue('G' . $cell, '-');
			$objSheet->setCellValueExplicit('H' . $cell, $row['keterangan'], PHPExcel_Cell_DataType::TYPE_STRING);
			$cell++;
			$no++;
		}
		...
		foreach ($data['debit'] as $row) {
			...
			$objSheet->setCellValue('A' . $cell, $no);
			$objSheet->setCellValue('B' . $cell, 'PENERIMAAN');
			...
			$objSheet->setCellValue('G' . $cell, '');
			$objSheet->setCellValueExplicit('H' . $cell, $row['debit_desc'], PHPExcel_Cell_DataType::TYPE_STRING);
			$cell++;
			$no++;
		}
		foreach ($data['kredit'] as $row) {
			...
			$objSheet->setCellValueExplicit('H' . $cell, $row['kredit_desc'], PHPExcel_Cell_DataType::TYPE_STRING);
			$cell++;
			$no++;
		}
		foreach ($data['gaji_pay'] as $row) {
			$objSheet->setCellValue('A' . $cell, $no);
			$objSheet->setCellValueExplicit('B' . $cell,  $row['guru_nama'], PHPExcel_Cell_DataType::TYPE_STRING);
			$objSheet->setCellValue('C' . $cell, '-');
			$objSheet->setCellValue('D' . $cell, '-');
			$objSheet->setCellValue('E' . $cell, pretty_date($row['gaji_pay_date'], 'm/d/Y', FALSE));
			$objSheet->setCellValue('F' . $cell,  '-');
			if ($row['gaji_jenis']== 'paket' ) {
				$objSheet->setCellValue('G' . $cell, $row['gaji_pay_value']);
			} else {
				$objSheet->setCellValue('G' . $cell, $row['gaji_pay_value']*$row['gaji_value']);
			}
			$objSheet->setCellValueExplicit('H' . $cell, $row['gaji_pay_desc'], PHPExcel_Cell_DataType::TYPE_STRING);
			$cell++;
			$no++;
		}
	...
	public function report_bill_detail()
	{
		...
		$objSheet->setCellValue('A1', 'REKAPITULASI KEKURANGAN DAN STATUS LUNAS PEMBAYARAN SISWA');
		$objSheet->setCellValue('A2', $data['setting_school']['setting_value']);
		...
		foreach ($data['student'] as $row) {
			...
			foreach ($data['bulan'] as $key) {
				if ($key['student_student_id'] == $row['student_student_id']) {
					$objSheet->setCellValue(getCell($alphdata) . $cell, ($key['bulan_status'] == 1) ? 'Lunas' : $key['bulan_bill'] - $key['bulan_bill_total']);
					$alphdata++;
				}
			}
